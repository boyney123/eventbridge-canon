...
export default function EventList() {
  ...
  const { source: selectedSource = '', detailType: selectedDetailType = '' } = Object.fromEntries(urlSearchParams.entries());
          ...
          <nav className="flex-1 px-2 space-y-1" aria-label="Sidebar">
            {Object.keys(eventsBySource).map((source) => (
              <Disclosure as="div" key={source} className="space-y-1">
                {({ open }) => {
                  const show = open || source == selectedSource;
                  return (
                    <>
                      <Disclosure.Button className=" bg-white text-gray-600 hover:bg-gray-100 hover:text-gray-900 group w-full flex items-center pl-2 pr-1 py-2 text-left text-sm font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500">
                      ...
                      {show && (
                        <Disclosure.Panel className="space-y-1" static>
                          {eventsBySource[source].schemas.map((event) => (
                            <div key={event.id} className={`pl-4 pr-2 py-2 hover:bg-gray-100 border-b border-gray-100 ${source === selectedSource && event === selectedDetailType ? 'bg-gray-100' : ''} `}>
                              <Link href={`/events/create?source=${source}&detailType=${event}`}>
                                <a className="group w-full items-center  text-sm font-medium text-gray-600 rounded-md hover:text-gray-900 hover:bg-gray-100 ">
